---
layout: post
title: 带进度条显示的异或加密程序
date: 2014-10-14
---

把算法弄复杂了一点，增加破解难度。

前 512 字节以计数器作为异或数，计数器以 128 为边界，反复初始化，根据 SETP 指定的步进来递增。

512 字节后面的内容根据奇偶数，分别与 PWD1 和 PWD2 作异或。

为了看起来高大上，特地用多线程写了一个显示进度的函数，可以正常工作，不过光标总是在进度条内跳来跳去，不爽。


```c
#include <stdio.h>
#include <string.h>
#include <pthread.h>
#include <unistd.h>

#define PWD1 xx
#define PWD2 xx
#define STEP xx

/* 打印进度条 */
void DisplayProgress(long current, long total) {
    int width = 50; /* 进度条宽度为 50 个字符 */
    int i;
    /* 清空行 */
    for (i = 0; i < width + 7; i++) {
        putchar('\b');
    }
    
    int LoopTimes = (int)((double)current / total * width);

    putchar('[');
    for (i = 0; i < LoopTimes; i++)
        putchar('#');
    for (i = 0; i < width - LoopTimes; i++)
        putchar('-');
    printf("] ");
    printf("%3.0f%%", (double)current / total * 100);    
}

void barStart(long cnt[])
{
    long *current, *total;
    current = ((long *)cnt);
    total = ((long *)cnt) + 1;

    while (*current != *total) {
        DisplayProgress(*current, *total);
        usleep(100000);
    }
    DisplayProgress(50, 50);
}

int main(int argc, char *argv[])
{
    char* in_file;
    char* out_file;
    
    if (argc == 1) {
        printf("Please specify a file to encrypt!\n");
        return 0;
    } else {
        in_file = argv[1]; /* 参数1指定输入文件 */
    }

    char out[strlen(in_file) + 5];

    /* 如果参数大于一个 */
    if (argc > 2) {
        out_file = argv[2]; /* 输出文件由参数 2 指定 */
    } else {
        strcpy(out, in_file);
        strcat(out, ".enc"); /* 如果没有参数2，在输入文件名后边加上 .enc 作为输出文件 */
        out_file = out;
    }

    long cnt[2] = {0, 0}; /* 一个数组，保存计数及文件长度, 因为多线程调用函数只能传递一个参数
                             只好把它们包装在一个数组中传递进去*/
    int cnt2 = 0;

    FILE *fpi, *fpo;
    fpi = fopen(in_file, "rb");
    fpo = fopen(out_file, "wb");
    
    if (fpi != NULL)
    {
        fseek(fpi, 0L, SEEK_END);  /* 将文件指针移到末尾 */
        cnt[1] = ftell(fpi);       /* 将文件长度填入数组第2个单元 */
        fseek(fpi, 0L, SEEK_SET);  /* 将文件指针重新移回文件开头 */

        /* 如果文件为空，退出 */
        if (cnt[1] == 0) {
            printf("The file is empty, exit!\n");
            fclose(fpi);
            fclose(fpo);
            return 0;
        }

        /* 创建一个单独的线程，用来显示进度条 */
        pthread_t id;
        pthread_create(&id, NULL, (void *)barStart, (void *)cnt);
        
        while (! feof(fpi)) {
            int b = fgetc(fpi);
            if (b != EOF) {
                if (cnt[0] < 512) {
                    if (cnt2 < 128) {
                        fputc(b ^ (cnt2 % 128), fpo);
                        cnt[0]++;
                        cnt2 += STEP;
                    } else {
                        cnt2 = 0;
                        fputc(b ^ (cnt2 % 128), fpo);
                        cnt[0]++;
                        cnt2 += STEP;
                    }
                } else {
                    if (cnt[0] % 2 == 0) {
                        fputc(b ^ PWD1, fpo);
                        cnt[0]++;
                    } else {
                        fputc(b ^ PWD2, fpo);
                        cnt[0]++;
                    }
                }
            }
        }
        pthread_join(id, NULL);
        printf(" Done!\n");
    }
    else {
        printf("Fail to open file %s!\n", in_file);
    }

    fclose(fpi);
    fclose(fpo);
    return 0;

}
```