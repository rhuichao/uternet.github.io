...
>笔记，这一次假定(will-stop? last-try)返回 #t，也就是说传递空列表'()给last-try函数，它是能正常结束，并返回一个值。但是我们又错了，程序会在这里无限递归下去：(eternity '())，永远也不会结束。那么我们刚刚的假设不是又错了吗——我们假定(will-stop? last-try) 返回 #t，也就是说给last-try传递一个空列表作为参数，last-try函数可以正常地结束。也就是说，will-stop?函数结论上无法写出来。图灵已经证明过这个问题是无解的了。。。
