...
`(looking a lat)`是什么，其中a是 `caviar`， lat是`(6 2 4 caviar 5 7 3)`
...
>`(keep-looking a 7 lat)`, 其中a是`caviar`，lat是`(6 2 4 caviar 5 7 3)`
...
`(keep-looking a 3 lat)`是什么，其中a是`caviar`，lat是`(6 2 4 caviar 5 7 3)`
...
`(looking a lat)` 是什么，其中a是`caviar`，lat是`(7 1 2 caviar 5 6 3)`
...
>shuffle 函数不是 total function, 因为它再一次交换了pair的元件，这意味着我们又重头开始了。
...
那我们来看一些例子。`(will-stop? f)`，其中f是`length`
