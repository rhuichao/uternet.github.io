...
`(rember a lat)`是什么，其中  
a是`mint`，   
lat是`(lamb chops and mint jelly)`
...
是什么，其中    
a是`mint`     
lat是`(lamb chops and mint flavored mint jelly)`
...
是什么，其中    
a是`cup`    
lat是`(coffee cup tea cup and hick cup)`
...
>`else`询问真假。下一行用参数a和`(cdr lat)`递归查询，a是`and`，`(cdr lat)`是`(lettuce  and tomato)`
...
>递归查询，a是`and`，`(cdr lat)`是`(and tomato)`
...
>没有！  
>我们现在知道当lat等于`(and tomato)`时，`(rember a lat)`的结果。  
>但是我们还不知道当lat等于  
>`(lettuce and tomato)`  
>以及  
>`(bacon lettuce and tomato)`  
...
o1是`chocolate`  
o2是`banana`  
lat是`(banana ice cream with chocolate topping)`
