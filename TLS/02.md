...
问a是否是lat的一个成员，其中a是`tea`，lat是`(coffee tea or milk)`

>真。

------------

问`(member? a lat)` 是真还是假，其中a是`poached`，lat是`(fried eggs and scrambled eggs)`
...
问`(member? a lat)`是什么 其中a是`meat`，lat是`(mashed potatoes and meat gravy)`
...
<div class="textbox">
    <h1>The First Commandment 第一戒</h1>
...
问a是`meat`，lat是`(mashed potatoes and meat gravy)`时  
...
`(member? a lat)`的值是什么，其中a是`meat`，lat是`(meat gravy)`
...
`(member? a lat)`的值是什么，其中a是`meat`，lat是`(and meat gravy)`

>\#t

------------

`(member? a lat)`的值是什么，其中a是`meat`，lat是`(potatoes and meat gravy)`

>\#t

------------

`(member? a lat)`的值是什么，其中a是`meat`，lat是`(mashed potatoes and meat gravy)`
...
>函数递归参数a和(cdr lat)，a是`meat`，(cdr lat)是`(potatoes and meat gravy)`。
...
>是，但是`(eq? (car lat) a)` 为否。函数递归参数a和(cdr lat)，a为`meat`，(cdr lat)为`(and meat gravy)`
...
`(member? a lat)`的值是什么，其中a是`liver`，lat是`(bagels and lox)`
...
`(member? a lat)`的值是什么，其中a是`liver`，lat是`()`
...
`(member? a lat)`的值是什么，其中a是`liver`，lat是`(lox)`
...
下面的代码是什么值，其中a是`liver`，lat是`(and lox)`
...
`(member? a lat)`的值是什么，其中a是`liver`，lat是`(and lox)`
...
下面的代码是什么值，其中a是`liver`，lat是`(bagels and lox)`
